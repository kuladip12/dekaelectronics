...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Topbar ── */
  .topbar { background: var(--navy); color: #fff; padding: 16px 24px; }
  .topbar-inner { max-width: 1200px; margin: 0 auto; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; }
  ...
  /* ── Tabs ── */
  .tabs { background: var(--navy-l); display: flex; gap: 0; overflow-x: auto; padding: 0 16px; border-bottom: 1px solid rgba(255,255,255,.06); }
  .tabs::-webkit-scrollbar { height: 3px; } .tabs::-webkit-scrollbar-thumb { background: rgba(255,255,255,.2); }
  .tab-btn { background: none; border: none; color: #94A8C0; padding: 13px 17px; font-size: 13px; font-weight: 600; cursor: pointer; white-space: nowrap; border-bottom: 3px solid transparent; font-family: var(--sans); transition: color .15s; }
  .tab-btn:hover { color: #fff; }
  .tab-btn.active { color: #fff; border-bottom-color: var(--amber); }

  /* ── Layout ── */
  .main { max-width: 1200px; margin: 0 auto; padding: 22px 18px 60px; flex: 1; }
  ...
  .card-title { font-size: 15px; font-weight: 700; color: var(--navy); display: flex; align-items: center; gap: 8px; margin-bottom: 16px; }
  ...
  .stat-value { font-family: var(--mono); font-size: 23px; font-weight: 700; color: var(--navy); margin-top: 5px; }
  ...
  .field input, .field select { width: 100%; padding: 9px 11px; border: 1px solid var(--border); border-radius: var(--radius-md); font-size: 13.5px; background: #fff; color: var(--text); font-family: var(--sans); transition: border-color .15s, box-shadow .15s; }
  ...
  /* ── Buttons ── */
  .btn { display: inline-flex; align-items: center; gap: 6px; border: none; border-radius: var(--radius-md); padding: 9px 16px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: var(--sans); transition: all .15s; white-space: nowrap; }
  ...
  .btn-ghost { background: none; border: none; color: var(--muted); padding: 5px 7px; border-radius: 5px; } .btn-ghost:hover { color: var(--red); background: var(--red-bg); }
  ...
  th { text-align: left; padding: 8px 10px; background: #F0EDE5; color: var(--navy); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; font-weight: 700; border-bottom: 2px solid var(--border); }
  td { padding: 9px 10px; border-bottom: 1px solid var(--border-l); vertical-align: middle; }
  ...
  .picker-dropdown { position: absolute; left: 0; right: 0; top: calc(100% + 5px); background: #fff; border: 1px solid var(--border); border-radius: var(--radius-md); box-shadow: var(--shadow-lg); z-index: 300; max-height: 280px; overflow-y: auto; }
  .picker-row { padding: 10px 14px; cursor: pointer; border-bottom: 1px solid var(--border-l); display: flex; justify-content: space-between; align-items: center; gap: 10px; transition: background .1s; }
  .picker-row:last-child { border-bottom: none; }
  .picker-row:hover { background: #F6F3EC; }
  ...
  .picker-clear { position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: var(--muted); font-size: 16px; line-height: 1; padding: 2px 4px; border-radius: 4px; }
  .picker-clear:hover { color: var(--red); background: var(--red-bg); }

  /* ── Product info preview card ── */
  .product-preview { background: #F0EDE5; border: 1px solid var(--border); border-radius: var(--radius-md); padding: 13px 15px; margin-top: 10px; }
  .preview-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 10px; }
  .preview-item .pi-label { font-size: 10.5px; color: var(--muted); font-weight: 700; text-transform: uppercase; letter-spacing: .04em; }
  .preview-item .pi-val { font-family: var(--mono); font-size: 13.5px; font-weight: 700; color: var(--navy); margin-top: 3px; }
  ...
  .pill { background: #EDEAE3; border: none; padding: 7px 15px; border-radius: 20px; font-size: 12.5px; font-weight: 700; color: var(--muted); cursor: pointer; font-family: var(--sans); }
  ...
  .bar-label { width: 80px; flex-shrink: 0; color: var(--muted); font-family: var(--mono); font-size: 11.5px; }
  .bar-track { flex: 1; background: #EFEBE2; border-radius: 4px; height: 14px; overflow: hidden; }
  .bar-fill { background: linear-gradient(90deg, var(--amber), var(--amber-d)); height: 100%; border-radius: 4px; transition: width .3s ease; }
  .bar-val { width: 95px; text-align: right; font-family: var(--mono); font-size: 12px; flex-shrink: 0; color: var(--navy); }
  ...
  .bill-meta { display: flex; justify-content: space-between; font-size: 12.5px; color: var(--muted); margin-bottom: 10px; }
  .bill-totals { border-top: 2px dashed var(--border); margin-top: 8px; padding-top: 8px; }
  .bill-row { display: flex; justify-content: space-between; font-size: 13.5px; padding: 2px 0; }
  ...
  table.bill-tbl th { background: none; border-bottom: 1px solid var(--border); padding: 4px; font-size: 10.5px; }
  table.bill-tbl td { padding: 5px 4px; border-bottom: none; font-size: 12.5px; }

  /* ── Modals ── */
  .modal-bg { position: fixed; inset: 0; background: rgba(22,38,63,.55); display: flex; align-items: flex-start; justify-content: center; padding: 30px 16px; z-index: 500; overflow-y: auto; }
  .modal-box { background: #fff; border-radius: var(--radius); max-width: 480px; width: 100%; padding: 26px; max-height: calc(100vh - 60px); overflow-y: auto; box-shadow: var(--shadow-lg); }
  .modal-actions { display: flex; gap: 10px; justify-content: center; margin-top: 16px; }

  /* ── Toast ── */
  .toast { position: fixed; bottom: 22px; right: 22px; background: var(--navy); color: #fff; padding: 11px 20px; border-radius: 9px; font-size: 13.5px; font-weight: 600; box-shadow: var(--shadow-lg); z-index: 700; animation: toastIn .25s ease; max-width: 320px; }
  ...
  /* ── Cart ── */
  .qty-inp { width: 62px; padding: 5px 6px; text-align: center; border: 1px solid var(--border); border-radius: 6px; font-size: 13px; font-family: var(--mono); }
  .price-inp { width: 92px; padding: 5px 7px; text-align: right; border: 1px solid var(--border); border-radius: 6px; font-size: 13px; font-family: var(--mono); }
  ...
  @media (max-width: 640px) {
    .brand h1 { font-size: 17px; } .topbar-cats { display: none; }
...
function LoginScreen({ onLoggedIn }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--navy)", padding: 20 }}>
      <div className="card" style={{ width: 360, marginBottom: 0 }}>
  ...
  return (
    <>
      <div className="grid cols-5" style={{ marginBottom: 18 }}>
        {[
          { label: "Total Products", val: state.products.length },
          { label: "Total Stock Units", val: totalUnits },
          { label: "Low / Out of Stock", val: low.length, alert: low.length > 0 },
          { label: "Today's Sales", val: money(todaySales) },
          { label: "This Month's Sales", val: money(monthSales) },
        ].map(s => (
      ...
      <div className="card">
        <div className="card-title">🏷️ Stock value by brand <span className="count">({brandRows.length} brand{brandRows.length === 1 ? "" : "s"})</span></div>
        {!brandRows.length ? (
          ...
          brandRows.map(([brand, v]) => (
            <div className="bar-row" key={brand}>
              <span className="bar-label" style={{ width: 110 }}>{brand}</span>
              <span className="bar-track"><span className="bar-fill" style={{ width: Math.max(3, (v.value / maxBrandVal) * 100) + "%" }} /></span>
              <span className="bar-val" style={{ width: 150 }}>{money(v.value)} · {v.qty} pcs</span>
            </div>
          ))
        )}
      </div>
      ...
      <div className="card">
        <div className="card-title">⚠️ Low stock &amp; out-of-stock</div>
        {!low.length ? (
          <div className="empty"><strong>All stocked up</strong>No products are currently low or out of stock.</div>
        ) : (
          <div className="table-wrap">
            <table>
              <thead><tr><th>Product</th><th>Brand</th><th>Category</th><th className="right">Qty left</th><th>Status</th><th></th></tr></thead>
              <tbody>
                {low.map(p => {
                  const st = stockStatus(p);
                  return (
                    <tr key={p.id} className={st === "out" ? "out-stock" : "low-stock"}>
                      <td><strong>{p.name}</strong>{p.stars ? <span className="stars" style={{ marginLeft: 6 }}>{"⭐".repeat(+p.stars)}</span> : null}</td>
                      <td>{p.brand || "—"}</td>
                      <td>{CAT_ICON[p.category] || "🔌"} {p.category}</td>
                      <td className="right mono">{p.quantity}</td>
                      <td>{st === "out" ? <span className="badge badge-red">Out of stock</span> : <span className="badge badge-amber">Low stock</span>}</td>
                      <td><button className="btn btn-amber btn-sm" onClick={() => onGoToPurchase(p.id)}>+ Add stock</button></td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      <div className="card">
        ...
        {!recent.length ? (
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Invoice</th><th>Date</th><th>Customer</th><th className="right">Total</th></tr></thead>
              <tbody>
                {recent.map(s => (
                  <tr key={s.id}>
...
export default function App() {
  ...
  return (
    <>
      ...
      <footer style={{ maxWidth: 1200, margin: "0 auto", padding: "0 18px 30px", color: "var(--muted)", fontSize: 11.5, textAlign: "center" }}>
